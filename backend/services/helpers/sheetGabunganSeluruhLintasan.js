...
async function generateSheetGabunganSeluruhLintasan(
  workbook,
  allSheetNamesLBR,
  suratDokumen,
  tanggalDari,
  ...
  const sheetName = "Gab Seluruh Lintasan";
  ...
  const sumF = (row) => allSheetNamesLBR.map(n => `'${n}'!F${row}`).join('+');
  const sumE = (row) => allSheetNamesLBR.map(n => `'${n}'!E${row}`).join('+');
  const sumI = (row) => allSheetNamesLBR.map(n => `'${n}'!I${row}`).join('+');
  const sumJ = (row) => allSheetNamesLBR.map(n => `'${n}'!J${row}`).join('+');
  ...
  sc("D7", `: GABUNGAN SELURUH LINTASAN`, { bold: true, size: 14 });
  ...
  sc("D10", `: SEMUA LINTASAN`, { bold: true, size: 14 });
  sc("B11", "NAMA KAPAL / GRT", { bold: true, size: 14 });
  sc("D11", `: GABUNGAN SEMUA KAPAL`, { bold: true, size: 14 });
  ...
  // Data rows (20-104) - BARIS 20 E20 DAN F20 DIKOSONGKAN, BARIS 21-22 MENGGUNAKAN FORMULA
...
module.exports = { generateSheetGabunganSeluruhLintasan };
